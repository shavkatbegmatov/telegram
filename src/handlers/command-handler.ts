...
import { getMainMenuKeyboard } from "../keyboards/menus.js"

export function setupCommandHandler(config: Config, ticketStore: TicketStore) {
    ...
    if (isAdmin) {
      await ctx.reply(
        `🛠️ *Admin Support Dashboard*\n\n` +
          ...
          `• Use \`/stats\` to view support statistics.`,
        { parse_mode: "Markdown" }
      )
      return
    }
  ...
  composer.command("help", async (ctx) => {
    ...
    if (isAdmin) {
      await ctx.reply(
        `📌 *Admin Instructions*\n\n` +
          ...
          `4️⃣ Use \`/stats\` to view ticket counts.`,
        { parse_mode: "Markdown" }
      )
      return
    }
    ...
    await ctx.reply(
      `💬 <b>Need Support?</b>\n\n` +
        ...
        `Use the menu buttons below to view your <b>Profile</b> or explore <b>Premium</b> features!`,
    ...
    await ctx.reply(
      `🆔 *Chat Information*\n\n` +
        ...
        (isAdmin
          ...
          : `💡 Share your Chat ID with support if requested.`),
  ...
  composer.command("reply", async (ctx) => {
    ...
    if (!text) {
      await ctx.reply(
        "⚠️ *Usage:* `/reply <user_id> <your reply text>`\n" +
          "Example: `/reply 123456789 Hello, your issue has been resolved!`",
        { parse_mode: "Markdown" }
      )
      return
    }
  ...
  composer.hears("👤 Profile", async (ctx) => {
    await ctx.reply(
      `👤 <b>Demo User Profile</b>\n` +
        ...
        `• <b>League:</b> Silver 🏅\n` +
        `----------------------------------------\n` +
        `<i>💡 Demo Version - Sample User Information</i>`,
      { parse_mode: "HTML", reply_markup: getMainMenuKeyboard() }
    )
  })
  ...
  composer.hears("⭐ Premium", async (ctx) => {
    await ctx.reply(
      `🌟 <b>Kaizen Premium (Demo Preview)</b> 🌟\n` +
        `----------------------------------------\n` +
        ...
        `----------------------------------------\n` +
        `🚀 <b>Status:</b> Premium Subscription Coming Soon!`,
      { parse_mode: "HTML", reply_markup: getMainMenuKeyboard() }
    )
  })
  ...
  composer.hears("💬 Support", async (ctx) => {
    await ctx.reply(
      `💬 <b>Support Mode Active</b>\n\n` +
        `Simply send your question, message, or feedback right here in chat!\n` +
        `Our support team will receive your message and reply as soon as possible.`,
      { parse_mode: "HTML", reply_markup: getMainMenuKeyboard() }
    )
  })
  ...
  composer.on("callback_query:data", async (ctx) => {
    ...
    if (data.startsWith("admin_prompt_reply:")) {
      ...
      await ctx.reply(
        `✍️ <b>Reply to User ID <code>${userChatId}</code></b>:\n\n` +
          `Copy and complete this command:\n` +
          `<code>/reply ${userChatId} your reply message here</code>`,
        { parse_mode: "HTML" }
      )
      return
    }
