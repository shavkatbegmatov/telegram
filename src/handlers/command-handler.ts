...
import {
  getMainMenuKeyboard,
  getTaskInlineKeyboard,
  getHabitInlineKeyboard,
  getPremiumSkeletonKeyboard,
...
export function setupCommandHandler(
  ...
  const supportModeUsers = new Set<number>()

  async function requireLinkedProfile(ctx: any) {
    const profile = await kaizenService.getProfileByChatId(ctx.chat.id)
    if (!profile) {
      await ctx.reply(
        `⚠️ *Kaizen Account Not Linked*\n\n` +
          ...
          `3. Send \`/start <TOKEN>\` or \`/link <TOKEN>\` right here.`,
        { parse_mode: "Markdown" }
      )
      return null
    }
  ...
  composer.command("start", async (ctx) => {
      ...
      if (result.success && result.profile) {
        await ctx.reply(
          `🎉 *Account Connected Successfully!*\n\n` +
            ...
            `Use the menu below to manage your tasks, habits, goals, or request live support.`,
          { parse_mode: "Markdown", reply_markup: getMainMenuKeyboard() }
        )
        return
      } else {
        await ctx.reply(`❌ ${result.error}`, { reply_markup: getMainMenuKeyboard() })
        return
      }
    }
    ...
    if (profile) {
      await ctx.reply(
        `👋 Welcome to *Kaizen Companion*, ${profile.full_name || profile.username}!\n` +
          `Select an option from the menu below:`,
        { parse_mode: "Markdown", reply_markup: getMainMenuKeyboard() }
      )
    } else {
      await ctx.reply(
        `👋 *Welcome to Kaizen Companion & Support!*\n\n` +
          ...
          `*Need assistance right away?* Click **💬 Support** below to chat with our team!`,
        { parse_mode: "Markdown", reply_markup: getMainMenuKeyboard() }
      )
    }
  })
  ...
  composer.command("link", async (ctx) => {
    ...
    if (result.success && result.profile) {
      await ctx.reply(
        `🎉 *Account Connected Successfully!*\n\n` +
          `Connected as *${result.profile.full_name || result.profile.username}*!`,
        { parse_mode: "Markdown", reply_markup: getMainMenuKeyboard() }
      )
    } else {
    ...
    try {
      ...
      await ctx.reply(
        `✨ *Task Created!*\n\n` +
          ...
          `🎯 *Status:* Pending`,
        { parse_mode: "Markdown", reply_markup: getTaskInlineKeyboard(newTask.id) }
      )
    } catch (err: any) {
  ...
  // Settings handler
  const sendSettingsMenu = async (ctx: any) => {
    const profile = await requireLinkedProfile(ctx)
    if (!profile) return
    ...
    await ctx.reply(
      `⚙️ *Duolingo Task Notification Settings*\n\n` +
        ...
        `Tap a quick button below to change your lead time preference:`,
      { parse_mode: "Markdown", reply_markup: getNotificationSettingsInlineKeyboard() }
    )
  }
  ...
  composer.hears("📋 Today's Tasks", async (ctx) => {
    ...
    for (const t of tasks) {
      const deadlineStr = t.deadline
        ...
        : ""
      await ctx.reply(
        `• *${t.title}* (+${t.xp_reward} XP)${deadlineStr}\nPriority: ${t.priority.toUpperCase()}`,
        { parse_mode: "Markdown", reply_markup: getTaskInlineKeyboard(t.id) }
      )
    }
  })
    ...
    for (const h of habits) {
      if (h.is_completed_today) {
        ...
        await ctx.reply(`⚡ *${h.title}* (+${h.xp_reward} XP) — Streak: ${h.streak} days`, {
          parse_mode: "Markdown",
          reply_markup: getHabitInlineKeyboard(h.id),
        })
      }
    }
  })
  ...
  composer.hears("📅 Weekly Review", async (ctx) => {
    ...
    await ctx.reply(
      `📅 *Weekly Performance Summary*\n\n` +
        ...
        `🔥 *Streak Status:* ${profile.streak} days active!`,
      { parse_mode: "Markdown" }
    )
  })
  ...
  composer.hears("💬 Support", async (ctx) => {
    ...
    await ctx.reply(
      `💬 *Support Mode Active*\n\n` +
        ...
        `Our support admins will receive your message and reply directly to you!`,
      { parse_mode: "Markdown" }
    )
  })
  ...
  composer.hears("⭐ Premium Membership", async (ctx) => {
    await ctx.reply(
      `🌟 *Kaizen Premium Membership* 🌟\n\n` +
        ...
        `*Status:* 🚀 Coming Soon!`,
      { parse_mode: "Markdown", reply_markup: getPremiumSkeletonKeyboard() }
    )
  })
    ...
    if (data.startsWith("complete_task:")) {
      ...
      try {
        ...
        await ctx.editMessageText(
          `✅ *Task Completed!*\n"${task.title}" (+${xpAwarded} XP awarded)`,
          { parse_mode: "Markdown" }
        )
      } catch (err: any) {
